...
checkMount
if [ -z "$efiloc" ]; then
	echo "efi mount failed, exit"
	exit 1
fi

checkAndCreateDir $temp_dir
if [ -d $temp_dir/CLOVER ]; then rm -r $temp_dir/CLOVER; fi
cp -R "$efiloc/EFI/CLOVER" "$temp_dir/"

if [ -f "$temp_dir/CLOVER/config.plist" ]; then
	if [ -f my_config.plist ]; then rm my_config.plist; fi
	cp $temp_dir/CLOVER/config.plist my_config.plist

	if [ -f config.plist ]; then rm config.plist; fi
	cp my_config.plist config.plist
fi

checkCommands $pledit
$pledit -c "Delete :RtVariables:ROM" -c "Add :RtVariables:ROM string UseMacAddr0" \
	-c "Delete :RtVariables:MLB" -c "Delete :SMBIOS:SerialNumber" -c "Add :SMBIOS:SerialNumber string" \
	-c "Delete :SMBIOS:SmUUID" -c "Add :SMBIOS:SmUUID string" -c "Delete :SystemParameters:CustomUUID" \
	-c "Add :SystemParameters:CustomUUID string" config.plist
